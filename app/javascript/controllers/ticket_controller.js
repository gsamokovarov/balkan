...
export default class extends Controller {
  ...
  countValueChanged() {
    if (this.countValue < 3) {
      this.summaryTarget.classList.remove("text-brand-600", "text-teal-600")
      if (this.countValue === 2) this.summaryTarget.classList.add("text-brand-600")
      this.summaryTarget.textContent = `Buy ${3 - this.countValue} more and save 10%`
    } else {
      this.summaryTarget.classList.remove("text-brand-600")
